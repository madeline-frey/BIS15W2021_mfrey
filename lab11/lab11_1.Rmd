...
homerange %>% 
  ...
  labs(title = "Distribution of Body Mass")
#Alpha means the transparency of the fill color!!
```
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.hra))+geom_histogram(binwidth=1, fill= "orchid4", alpha=.25, color= "white")+
  labs(title= "Homerange Size", x="Log 10 Homerange Area", y="Count")+
  theme(plot.title = element_text(size = rel(1.25), hjust = 0.5))
#If you leave out alpha, the fill will be completely opaque. 
```

2. Now plot the same variable using `geom_density()`.
```{r}
homerange %>% 
  ggplot(aes(x=log10.hra))+geom_density(fill= "darkolivegreen", alpha=.7, color= "chocolate")
```
...
homerange %>% 
  ...
  geom_histogram(aes(y = ..density..),binwidth=1, fill= "tomato", alpha=.5, color= "white")+
  geom_density(color= "darkolivegreen")
  labs(title= "Homerange Size", x="Log 10 Homerange Area", y="Count")+
  theme(plot.title = element_text(size = rel(1.25), hjust = 0.5))
```
...
homerange %>% 
  ...
  geom_bar() +
  coord_flip() +
  labs(title = "Observations by Class and Range Category in Homerange Data",
       x = "Range Category",
       fill = "Trophic Guild")
```
