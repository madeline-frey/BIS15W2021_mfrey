...
p
#store basic plot as an object, then add more stuff later.
```
...
p+theme_linedraw()+
  ...
  labs(title = "Observations by Taxon in Homerange Data",
       ...
       y= "n",
       fill= "Trophic Guild")
#using legend.position='none' removes legend. 
```
...
```{r}
?RColorBrewer
#3 different types of palettes for sequential, diverging, and qualitative data. 
```
...
#number is the number of variables/values. 
```
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.mass, y=log10.hra, color= class))+point_color_manual(my_palette)
```
...
#grid is used for 2 separate categorical variables
```
## Printing
```{r}
pdf("homerange.pdf")#title of file must have file type on it
p2
dev.off()#close the device (PDF)
```
