...
i <- 1 # what is the purpose of i?
repeat{
  iris1[ , i] <- iris1[ , i] * 10
  i <- i + 1
  if(!is.numeric(iris1[ , i])){ # break when you get to species.
    break
  }
}
head(iris1)
...
i <- 1
while(is.numeric(iris2[ , i])){ #different way to break at species
  iris2[ , i] <- iris2[ , i] * 10
...
for(i in c(2,4)){#why is this a different format? Also where does the 4 come in?
  iris3[ ,i] <- iris[ ,i]*10
}
iris3
```
...
for(i in 1:ncol(iris3)){
  if(str_detect(colnames(iris3)[i], "Width")){# using name detection instead of entering column
    iris3[ , i] <- iris3[ , i] * 10
  }
}
head(iris3)
