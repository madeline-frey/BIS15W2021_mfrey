...
#apps can be run within R(for people who are familiar) or externally for others to be able to access. 
```

```{r}
if (!require("tidyverse")) install.packages('tidyverse')
#if the code requires a package, it will automatically install.
```
...
```{r, eval=F, echo=T}
#to make this run externally: add libraries and data in new RScript.
ui <- fluidPage(    
...
server <- function(input, output, session) {#session part needs to be present. 
  session$onSessionEnded(stopApp) #stops the app once the session is closed. 
