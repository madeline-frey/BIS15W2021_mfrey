...
```{r}
penguins %>% 
  count(island, species, sort = F) # if sort= T sorted in descending order, what does sort= F mean? no sorting?
```
...
```{r}
penguins %>%
  summarize(across(contains("mm"), mean, na.rm=T)) #na.rm translates to "na remove, so true is saying do remove them. 
```
