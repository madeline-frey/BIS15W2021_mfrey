...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=order, y=mean_mass))+geom_col()+coord_flip()+scale_y_log10() # scale log to better view very large and very small
```
...
#non-finite values mean NA? But it already removed NA?
```
...
#Is there a way to get a bendy line?
```
...
#Se=F means don't put standard error on the line?
```
