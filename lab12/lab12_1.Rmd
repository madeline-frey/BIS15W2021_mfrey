...
```{r}
spider_list# Are we supposed to do the same thing as before? I'm confused
```
...
```{r}
names_vec <- unlist(names_list) #what does unlist do specifically?
names_vec
```
```{r}
?unlist() # createsa vector with all of the actual components from the list. 
```
...
full_exampleDF
#why do the other join types exclude certain missing data?
```
...
```{r}
mdy(dateformat2) # why does the month come last....... confusion
```
