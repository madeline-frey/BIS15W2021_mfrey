...
```{r}
lat <- c(34.67, 41.80)
long <- c(-124.1, -115.5)# uses min and max lat and long
bbox <- make_bbox(long, lat, f = 0.05) #f means slightly expanding box beyond coordinates
```
...
```{r}
ggmap(map2)+geom_point(data=spiders, aes(longitude, latitude, color=county, alpha=.5),size=.5)+
  labs(x = "Longitude", y = "Latitude", title = "Spider Locations", color= "County")
  #some of the stamen base maps don't seem to work with the data. EX: toner-hybrid caused a bunch of spiders to be in the ocean.
```
...
```{r}
#only works for states in the lower 48!
library(albersusa)
```
...
```{r}
spiders %>% 
 mutate(filter(str_detect(locality, "Cave"))
```
